...
def handler(event, context):
    try:
        ...
        try:
            ...
            if event["detail-type"] == "Scheduled Event":
                ...
                current_date_str = (current_date_obj_utc-datetime.timedelta(days=1)).strftime('%-m/%-d/%y')
                ...
                if current_date_obj_pst.weekday() == 0:
                    sun_date_str = (current_date_obj_utc-datetime.timedelta(days=2)).strftime('%-m/%-d/%y')
                    error_message_sun, results = updateBuilding(sun_date_str,trace_mode="historical")
                    print(results)
                    print("sunday string",sun_date_str, " sunday error message ", error_message_sun)
                    if error_message_sun:
                        print(error_message_sun)
                        message = error_message_sun
                        status_code = 400
                # update normal weekdays + Saturday
                error_message_multi, results = updateBuilding(current_date_str,trace_mode="multi")
                error_message_historical, results = updateBuilding(current_date_str,trace_mode="historical")
    ...
    if  input_pass != service_password:
        ...
        if input_mode == "affected_buildings":
            ...
            error_message, results = updateBuilding(input_date,trace_mode="multi")
        elif input_mode == "historical":
            error_message, results = updateBuilding(input_date,trace_mode="historical")
