...
class Trace:
    ...
    def get_manhole_map(self, date_value, mode="detection"):
        ...
        for _,row in day_data.iterrows():
            try:
                ...
                day_map[row["ManholeID"]] = int(float(row[date_value]) > 0) or sm_tag
            except:
                empty_count += 1
        ...
        if empty_count == len(day_data): raise InvalidDateError
        return day_map
    ...
    def get_affected_buildings(self,date_value,mode="detection"):
        ...
        """
        affected_buildings = []
        error_message, pos_mh_list = self.get_positive_manholes(date_value,mode)
    ...
    def get_affected_manholes(self,date_value,mode="detection"):
        ...
        affected_manholes = []
        error_message, pos_mh_list = self.get_positive_manholes(date_value,mode)
        if error_message: return error_message, affected_manholes
        error_message,barriers = self.get_negative_barriers(date_value,mode)
        if error_message: return error_message, affected_manholes
        self.mh_graph.barriers = barriers
        self.mh_graph.buildGraph()
    ...
    def exportDropIn(self,date_value):
        ...
        saved_path = '/tmp/dropin%s.csv'%("".join(date_value.split("/"))+str(year))
        error_message,barriers = self.get_negative_barriers(date_value)
        if error_message: return error_message, saved_path
        self.mh_graph.barriers = barriers
        self.mh_graph.buildGraph()
        ...
        drop_in = pd.merge(full_mh_trace_df,drop_in, on='MANHOLE_ID',how='left')
    ...
    def MultiTraceManholes(self,date_val):
        ...
        for mode, _ in mode_col_map.items():
            error_message, affected_manholes = self.get_affected_manholes(date_val,mode=mode)
